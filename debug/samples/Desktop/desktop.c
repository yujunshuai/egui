...
static si_t desktop_default_widget_repaint(struct window * w, union message * msg)
{
	...
	for(int i=0;i<bar_num;i++){
		...
		if(strcmp(win_info_ptr->title,"time")==0)
			continue;
		--i;//为了跳过对time的任务栏图标的绘制
		set_desktop_color(w,&bar_blue);
		fill_rectangle(w->gd,(i+1)*80,1+screen_h-30,79,28);

		set_desktop_color(w, &font_white);
		if(strstr(win_info_ptr->title,"cal")!=NULL){
			image=image_view_init("/home/yu/egui/resource/icons/calculator.bmp");
		}else if(strstr(win_info_ptr->title,"gam")!=NULL){
			image=image_view_init("/home/yu/egui/resource/icons/game.bmp");
		}else if(strstr(win_info_ptr->title,"fil")!=NULL){
			image=image_view_init("/home/yu/egui/resource/icons/file_browser.bmp");
		}else if(strstr(win_info_ptr->title,"pop")!=NULL){
			image	=image_view_init("/home/yu/egui/resource/icons/pop_up_window.bmp");
		}else if(strstr(win_info_ptr->title,"sin")!=NULL){
			image=image_view_init("/home/yu/egui/resource/icons/single_window.bmp");
		}
		image_view_set_bounds(image,(i+1)*80,1+screen_h-30,79,28);
	//	object_attach_child(OBJECT_POINTER(w),OBJECT_POINTER(image));
		image_view_reshow(image);
